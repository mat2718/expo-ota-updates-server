...
export async function getExpoConfigAsync({
  ...
  try {
    ...
    const expoConfigBuffer = await fs.readFile(path.resolve(expoConfigPath), { encoding: 'utf-8' });
    const expoConfigJson = JSON.parse(cleanString(expoConfigBuffer.toString().replace(/\0/g, '')));
